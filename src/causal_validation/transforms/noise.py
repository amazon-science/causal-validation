...
class Noise(AdditiveTransform):
    noise_dist: TimeVaryingParameter = TimeVaryingParameter(sampling_dist=norm(0,.1))
    ...
    def get_values(self, data: Dataset) -> Float[np.ndarray, "N D"]:
        ...
        noise_treatment = self.noise_dist.get_value(n_units=1, n_timepoints=data.n_timepoints).reshape(-1)
